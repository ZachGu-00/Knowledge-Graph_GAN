...
class AdversarialTrainingPipeline:
    ...
    def _load_multiline_json(self, file_path: str) -> List[Dict]:
        ...
        print(f"Loaded {len(data)} JSON objects from {file_path}")
        return data
        ...
        for epoch in range(epochs):
            ...
            try:
                ...
                if (epoch + 1) % 2 == 0:  # 每2个epoch保存一次
                    try:
                        from pathlib import Path
                        checkpoint_dir = "checkpoints/adversarial/periodic"
                        Path(checkpoint_dir).mkdir(parents=True, exist_ok=True)
                        
                        self.trainer.save_checkpoint(
                            f"{checkpoint_dir}/epoch_{epoch+1}_checkpoint.pth", 
                            epoch + 1
                        )
    ...
    def _check_and_save_best_model(self, epoch_metrics, epoch):
        ...
        if current_score > best_score:
            ...
            try:
                ...
                # 创建保存目录
                best_epoch_dir = f"checkpoints/adversarial/best_epoch_{epoch+1}"
                Path(best_epoch_dir).mkdir(parents=True, exist_ok=True)
                
                # 保存最佳模型
                self.trainer.save_checkpoint(
                    f"{best_epoch_dir}/model_checkpoint.pth", 
                    epoch + 1
                )
                print(f"💎 Best model saved: {best_epoch_dir}")
                
                # 也保存到主adversarial目录
                Path("checkpoints/adversarial").mkdir(parents=True, exist_ok=True)
                self.trainer.save_checkpoint(
                    "checkpoints/adversarial/latest_best_model.pth", 
                    epoch + 1
                )
                print(f"🔄 Adversarial models updated: checkpoints/adversarial/")
    ...
    def _finalize_training(self):
        ...
        try:
            ...
            # 创建最终模型目录
            final_dir = "checkpoints/adversarial/final"
            Path(final_dir).mkdir(parents=True, exist_ok=True)
            
            # 保存最终模型
            self.trainer.save_checkpoint(
                f"{final_dir}/final_model_checkpoint.pth", 
                len(self.training_log['epoch_metrics'])
            )
            print(f"Final adversarial models saved: {final_dir}")
    ...
    def _display_final_report(self):
        ...
        print("\nNEXT STEPS:")
        print("  1. Check saved models in: checkpoints/adversarial/")
        print("  2. View training logs in current directory")
        print("  3. Best model saved in: checkpoints/adversarial/latest_best_model.pth")
        print("  4. Final model saved in: checkpoints/adversarial/final/final_model_checkpoint.pth")
        print("="*80)
